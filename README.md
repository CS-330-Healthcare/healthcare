...
There are four main tasks that our group decided to test and support using our app. The first task we implemented is to create a user account and input relevant user information, such as insurance information and login credentials, into the app. The second is to login to an existing user profile to view your profile information. This can be done using a sample login with username and password "bojack" and we also have made another profile with the username "diane". The next task is to view primary care doctors in your area and find one to schedule an appointment with that is in your insurance network. The final task is to find the least expensive place in your area to get a flu shot.
